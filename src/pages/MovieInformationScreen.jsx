...
const TestDetail = () => {
  ...
  return (
    <div className='min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12' key={detail?.data?.id}>
      <div className='pb-3 sm:max-w-4xl sm:mx-auto'>
        <div className='bg-white shadow-lg border-gray-100 min-h-[500px] border sm:rounded-3xl flex justify-between'>
          <div className=' overflow-visible w-[45%] p-1'>
            <img
              className=' rounded-l-2xl shadow-lg mx-auto h-full bg-center object-cover'
          ...
          <div className='flex flex-col w-[50%] p-6 rounded-md'>
            ...
            <div className='text-lg  items-center justify-between text-gray-800'>
              <p className=' italic font-bold mt-3'>{detail?.data?.release_date.split('-').slice(0, 1)}</p>
              <button
                className='bg-yellow-500 text-white p-2 mt-3 rounded-md  text-sm hover:bg-yellow-700 hover:duration-200 hover:transition-all'
            ...
            <p className=' text-gray-400 mt-5 max-h-40 overflow-y-auto'>{detail?.data?.overview}</p>
          </div>
        </div>
